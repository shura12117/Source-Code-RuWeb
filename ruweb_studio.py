...
# ==================== ПОДСВЕТКА СИНТАКСИСА (ИСПРАВЛЕННАЯ) ====================
class SyntaxHighlighter:
    """Контекстная подсветка: разметка / CSS / JS"""
    TAGS = {
        'страница','голова','тело','заголовок','мета','стили','сценарий',
        'секция','заголовок_блок','подвал','основной','навигация','боковая',
        'статья','заголовок1','заголовок2','заголовок3','заголовок4',
        'заголовок5','заголовок6','абзац','блок','строчный','перенос',
        'горизонтальная','список','список_нум','элемент','таблица','строка',
        'ячейка','ячейка_заг','голова_табл','тело_табл',
        'изображение','ссылка','форма','ввод','кнопка',
        'метка','текст_область','выбор','опция',
        'жирный','важный','курсив','акцент','подчеркнутый',
        'зачеркнутый','маленький','код','цитата','фрейм',
        'видео','аудио','источник','холст','детали','сводка',
        'диалог','прогресс','шаблон'
    }
    ATTRS = {
        'класс','ид','стиль','язык','кодировка','тип','имя',
        'значение','подсказка','при_клике','ссылка','источник',
        'альтернатива','ширина','высота','обязательный','отключен',
        'цель','при_изменении','при_отправке','при_загрузке',
        'автовоспроизведение','управление','зацикленный','загрузка',
        'только_чтение','автофокус','макс_длина','мин_длина',
        'при_наведении','при_уходе','при_фокусе','при_потере_фокуса',
        'при_нажатии','при_вводе'
    }
    KEYWORDS = {
        'перем','конст','функция','возврат','если','иначе_если',
        'иначе','для','пока','цикл','когда','истина','ложь',
        'прервать','продолжить','попробовать','поймать_ошибку',
        'наконец','новый','этот','асинхронная','ожидать'
    }
    FUNCS = {
        'вывести','предупредить','спросить','подтвердить',
        'получить_по_ид','получить_элемент','получить_все',
        'слушать','перестать_слушать','интервал','таймер',
        'остановить_интервал','остановить_таймер',
        'округлить','случайное','максимум','минимум',
        'целое','дробное','в_JSON','из_JSON','новая_дата',
        'текст','внутренний_HTML'
    }
    CSS_PROPS = {
        'шрифт-семейство','шрифт-размер','шрифт-вес','шрифт-стиль',
        'высота-строки','межбуквенный-интервал',
        'текст-выравнивание','текст-преобразование','текст-оформление','текст-тень',
        'граница-радиус','граница-ширина','граница-стиль','граница-цвет',
        'граница-сверху','граница-снизу','граница-слева','граница-справа',
        'отступ-снизу','отступ-сверху','отступ-слева','отступ-справа',
        'поля-снизу','поля-сверху','поля-слева','поля-справа',
        'макс-ширина','мин-ширина','макс-высота','мин-высота',
        'отступ','поля','фон','цвет-фона','фон-изображение',
        'фон-позиция','фон-размер','фон-повтор','цвет',
        'ширина','высота','граница','курсор','дисплей','позиция',
        'тень','прозрачность','видимость','переполнение',
        'переполнение-х','переполнение-у',
        'лево','право','верх','низ','z-индекс',
        'трансформация','переход','анимация','анимация-задержка',
        'фильтр','фильтр-фона','маска',
        'флекс','флекс-направление','флекс-обёртка','флекс-основа',
        'флекс-рост','флекс-сжатие',
        'выравнивание-элементов','выравнивание-содержимого','выравнивание-себя',
        'justify-содержимое','gap',
        'сетка-шаблон-колонки','сетка-шаблон-строки',
        'сетка-колонка','сетка-строка',
        'размер-блока','контур','стиль-списка','соотношение-сторон',
        'мин-высота'
    }
    CSS_VALUES = {
        'в-рамку','border-box','content-box','none','auto','inherit',
        'initial','unset','block','inline','inline-block','flex',
        'grid','relative','absolute','fixed','sticky','static',
        'bold','italic','normal','center','left','right','justify',
        'solid','dashed','dotted','hidden','visible','scroll','overlay',
        'row','column','wrap','nowrap','space-between','space-around',
        'space-evenly','stretch','baseline','start','end','between'
    }
    ...
    def __init__(self, t):
        ...
        for tag, fg in self._colors.items():
            self.t.tag_configure(tag, foreground=fg)
    
    def highlight(self, event=None):
        for tag in self._colors.keys():
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            # Смена контекста: стили
            if stripped.startswith('стили') and not in_script:
                in_style = True
                style_indent = indent
                self._add(line_num, indent, indent + len('стили'), 'tag')
                continue
            # Смена контекста: сценарий
            elif stripped.startswith('сценарий') and not in_style:
                in_script = True
                script_indent = indent
                self._add(line_num, indent, indent + len('сценарий'), 'tag')
                continue
            
            # Выход из блока по отступу
            if in_style and stripped and indent <= style_indent:
                in_style = False
            if in_script and stripped and indent <= script_indent:
                in_script = False
            ...
            if in_style:
                self._highlight_css(line_num, line)
            elif in_script:
                self._highlight_js(line_num, line)
            else:
                self._highlight_markup(line_num, line)
    
    def _highlight_markup(self, line_num, line):
        stripped = line.strip()
        indent = len(line) - len(line.lstrip())
        
        # Комментарий
        if stripped.startswith('#') or stripped.startswith('//'):
            self._add(line_num, indent, len(line), 'comment')
            return
        # Закрывающий тег
        if stripped.startswith('/'):
            ...
            return
        # документ_html
        if stripped == 'документ_html':
            self._add(line_num, indent, len(line), 'doc')
            return
        
        # Тег в начале строки
        parts = stripped.split(maxsplit=1)
        tag_name = parts[0].rstrip('/')
        tag_end_col = indent + len(tag_name)
        if tag_name in self.TAGS:
            self._add(line_num, indent, tag_end_col, 'tag')
        
        # Атрибуты и строки после тега
        if len(parts) > 1:
            rest = parts[1]
            # Найти начало rest в исходной строке
            rest_start = line.find(rest, tag_end_col)
            if rest_start == -1:
                rest_start = tag_end_col
            self._highlight_attrs(line_num, rest, rest_start)
    
    def _highlight_attrs(self, line_num, text, start_col):
        i = 0
        while i < len(text):
            if text[i].isspace():
                i += 1
            ...
            if text[i] == '"':
                ...
                self._add(line_num, start_col + i, start_col + end + 1, 'string')
                i = end + 1
                continue
            # Атрибут=значение
            m = re.match(r'([a-zA-Zа-яА-Я_][\w-]*)\s*=', text[i:])
            if m:
                attr_name = m.group(1)
                attr_start = start_col + i
                attr_end = attr_start + len(attr_name)
                if attr_name in self.ATTRS:
                    self._add(line_num, attr_start, attr_end, 'attr')
                eq_pos = text.find('=', i)
                if eq_pos != -1:
                    self._add(line_num, start_col + eq_pos, start_col + eq_pos + 1, 'punct')
                i = eq_pos + 1
                continue
            i += 1
    
    def _highlight_css(self, line_num, line):
        """Подсветка CSS внутри блока стили.
        Формат: | селектор { свойство: значение; ... }
        или:    | свойство: значение;
        """
        stripped = line.strip()
        if not stripped.startswith('|'):
            return
        
        # Найти позицию | в исходной строке (это и есть колонка в Tkinter)
        pipe_col = line.index('|')
        self._add(line_num, pipe_col, pipe_col + 1, 'css_punct')
        
        # Содержимое после | (без ведущих пробелов после |)
        after_pipe = line[pipe_col + 1:]
        css_content = after_pipe.strip()
        
        if not css_content:
            return
        
        # Найти позицию начала css_content в исходной строке
        content_start = line.index(css_content, pipe_col + 1)
        
        # Ищем фигурные скобки
        brace_open = css_content.find('{')
        brace_close = css_content.rfind('}')
        
        if brace_open != -1 and brace_close != -1 and brace_close > brace_open:
            # === Селектор до { ===
            sel_text = css_content[:brace_open].strip()
            if sel_text:
                sel_start_in_css = css_content.index(sel_text)
                sel_start = content_start + sel_start_in_css
                self._add(line_num, sel_start, sel_start + len(sel_text), 'css_sel')
            
            # === Свойства внутри { } ===
            props_text = css_content[brace_open + 1:brace_close]
            props_offset = content_start + brace_open + 1  # смещение props_text в line
            
            # Регулярка для свойств: имя: значение;
            # Имя свойства может содержать русские буквы, латиницу, цифры, дефис
            for pm in re.finditer(r'([a-zA-Zа-яА-ЯёЁ][\wа-яА-ЯёЁ-]*)\s*:\s*([^;]+?)(?:;|$)', props_text):
                prop_name = pm.group(1).strip()
                prop_val = pm.group(2).strip()
                
                # Позиция имени свойства
                prop_start = props_offset + pm.start(1)
                prop_end = prop_start + len(pm.group(1))
                self._add(line_num, prop_start, prop_end, 'css_prop')
                
                # Позиция значения
                val_start = props_offset + pm.start(2)
                val_end = val_start + len(pm.group(2))
                
                # Подсветка значения в зависимости от типа
                if prop_val.startswith('#') and re.match(r'^#[0-9a-fA-F]{3,8}$', prop_val):
                    self._add(line_num, val_start, val_end, 'css_val')
                elif 'градиент' in prop_val.lower() or 'linear' in prop_val.lower() or 'radial' in prop_val.lower():
                    self._add(line_num, val_start, val_end, 'css_val')
                elif re.match(r'^-?\d+(\.\d+)?(px|em|rem|%|vh|vw|vmin|vmax|pt|cm|mm|in|ex|ch)?$', prop_val):
                    self._add(line_num, val_start, val_end, 'css_num')
                elif prop_val.lower() in self.CSS_VALUES:
                    self._add(line_num, val_start, val_end, 'css_val')
                elif prop_val.startswith("'") or prop_val.startswith('"'):
                    self._add(line_num, val_start, val_end, 'string')
                elif re.match(r'^\d+$', prop_val):
                    self._add(line_num, val_start, val_end, 'css_num')
        else:
            # Нет фигурных скобок — просто свойство: значение
            m = re.match(r'\s*([a-zA-Zа-яА-ЯёЁ][\wа-яА-ЯёЁ-]*)\s*:\s*(.+?)\s*;?\s*$', css_content)
            if m:
                prop_name = m.group(1)
                prop_val = m.group(2).strip()
                
                prop_start_in_css = css_content.index(prop_name)
                prop_start = content_start + prop_start_in_css
                self._add(line_num, prop_start, prop_start + len(prop_name), 'css_prop')
                
                colon_in_css = css_content.index(':', prop_start_in_css)
                val_start_in_css = colon_in_css + 1
                # Пропустить пробелы после двоеточия
                while val_start_in_css < len(css_content) and css_content[val_start_in_css] == ' ':
                    val_start_in_css += 1
                val_start = content_start + val_start_in_css
                val_end = val_start + len(prop_val)
    ...
    def _highlight_js(self, line_num, line):
        stripped = line.strip()
        indent = len(line) - len(line.lstrip())
        ...
        # Защищаем строки от парсинга
        strings = []
        def save_string(m):
            strings.append(m.group(0))
            return f'\x00{len(strings)-1}\x00'
        protected = re.sub(r'"[^"]*"', save_string, line)
        protected = re.sub(r"'[^']*'", save_string, protected)
        
        # Ключевые слова
        for kw in self.KEYWORDS:
            for m in re.finditer(r'\b' + re.escape(kw) + r'\b', protected):
                self._add(line_num, m.start(), m.end(), 'keyword')
        
        # Функции
        for fn in self.FUNCS:
            for m in re.finditer(r'\b' + re.escape(fn) + r'\b', protected):
                self._add(line_num, m.start(), m.end(), 'func')
        
        # Числа
        for m in re.finditer(r'\b\d+(\.\d+)?\b', protected):
            self._add(line_num, m.start(), m.end(), 'num')
        
        # Скобки и операторы
        for m in re.finditer(r'[()\[\]{}=+\-*/<>!]', protected):
            self._add(line_num, m.start(), m.end(), 'punct')
        
        # Строки поверх всего
        for idx, s in enumerate(strings):
            pos = line.find(s)
            if pos != -1:
                self._add(line_num, pos, pos + len(s), 'string')
    
    def _add(self, line_num, start_col, end_col, tag):
        try:
            self.t.tag_add(tag, f'{line_num}.{start_col}', f'{line_num}.{end_col}')
        except Exception:
            pass
...
class RuwebStudio:
    ...
    def __init__(self):
        ...
        # Плавное появление: начинаем с полной прозрачности
        ...
        self.root.after(150, self._fade_in)
    
    def _fade_in(self):
        """Плавное появление окна через увеличение прозрачности"""
        try:
